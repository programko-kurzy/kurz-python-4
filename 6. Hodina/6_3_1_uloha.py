...
for i in range(6):
    coin_collect.append(pygame.image.load('./coin_collect/tile00'+str(i)+'.png').convert_alpha())
...
coins_being_collected = []
coins_being_collected_frame = []
...
def check_collision():
    ...
    for i in range(len(coins)):
        if player_rect.colliderect(coins[i]):
            moje_mince += 1
            coins_being_collected.append(coin_collect[0].get_rect(center = (coins[i].x, coins[i].y)))
            coins_being_collected_frame.append(0)
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == ANIMATION_EVENT:
            current_frame += 1
            for i in range(len(coins_being_collected_frame)):
                coins_being_collected_frame[i] += 1
                if (coins_being_collected_frame[i] > 5):
                    coins_being_collected_frame.pop(i)
                    coins_being_collected.pop(i)
    ...
    for i in range(len(coins_being_collected)):
        screen.blit(coin_collect[coins_being_collected_frame[i]], coins_being_collected[i])
