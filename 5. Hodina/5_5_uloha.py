...
ghosts = []
for i in range(5):
    ghosts.append(ghost.get_rect(center = (randint(0, 1024), randint(0, 720))))
...
def check_collision():
    global player_rect, medals, ghosts, moje_medaile
    ...
    for i in range(len(ghosts)):
        if player_rect.colliderect(ghosts[i]):
            moje_medaile -= 1
            player_rect.update(0, 0, player_rect.width, player_rect.height)
            ghosts[i].update(randint(0, 1024), randint(0, 720), ghosts[i].width, ghosts[i].height)
            print("Narazil si na ducha a stratil medailu!")
            print("Aktuálny počet medailí:", moje_medaile)
